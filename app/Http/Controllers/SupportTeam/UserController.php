...
class UserController extends Controller
{
    ...
    public function edit($id)
    {
        ...
        $d['cities'] = $this->loc->getCities($d['user']->prov_id);
        $d['districts'] = $this->loc->getDistricts($d['user']->city_id);
        $d['subdistricts'] = $this->loc->getSubDistricts($d['user']->dis_id);
        $d['provinces'] = $this->loc->getAllProvinces();
        $d['users'] = $this->user->getPTAUsers();
    ...
    public function update(UserRequest $req, $id)
    {
        ...
        if($user_is_staff){
            $d2 = $req->only(Qs::getStaffRecord());
            $d2['code'] = Qs::getAppCode().'/STAFF/'.date('Y/m', strtotime($req->emp_date)).'/'.mt_rand(1000, 9999);
